...
Soralink は OSS として開発します。実装、Issue、Pull Request、設定例、デプロイ手順は公開される前提のため、API key、Agent token、Stripe secret、Supabase secret/service role key などの秘匿情報をリポジトリに含めない運用を必須とします。
...
- [技術スタック](docs/tech-stack.md)
  - フロントエンド、Go Relay/Agent、Supabase、Stripe、VPS デプロイ、CI/セキュリティツール。
...
3. Supabase Auth の GitHub OAuth ログイン、トークン発行、ダッシュボードを作る
4. Supabase Postgres + RLS でユーザーデータを保護する
5. カスタムドメイン、アクセス制御、ログ/メトリクスを足す
6. Stripe でプラン制限と課金へ拡張する

初期インフラは、開発者が所有しているグローバル IP 付き VPS 1 台を Relay として利用します。SaaS として育てる場合でも、最初の実装は「単一 VPS Relay + Supabase + Stripe」で始めると、ネットワーク部分を小さく検証できます。
